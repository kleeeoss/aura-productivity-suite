...
import { CheckCircle, Clock, FileText, BookOpen, Activity, Calendar as CalendarIcon, Plus, Play, PenTool, Flame, Target } from 'lucide-react';
...
const Dashboard = () => {
  ...
  // Use a built-in array of quotes for reliability and speed as requested
  const builtInQuotes = [
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', height: '100%', paddingBottom: '40px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <p style={{ color: 'var(--text-secondary)' }}>{format(new Date(), 'EEEE, MMMM do, yyyy')}</p>
              <LiveClock />
            </div>
          </div>
        </div>
      ...
      {quote && (
        <GlassPanel style={{ padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
          <p style={{ fontStyle: 'italic', color: 'var(--text-secondary)' }}>"{quote.content}" <span style={{ fontWeight: '500', color: 'var(--text-primary)' }}>— {quote.author}</span></p>
        </GlassPanel>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
        ...
        <GlassPanel style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          ...
          <div style={{ marginTop: '16px' }}>
            <div style={{ fontSize: '2rem', fontWeight: 'bold' }}>{pomodorosCompletedToday} <span style={{ fontSize: '1rem', fontWeight: 'normal', color: 'var(--text-secondary)' }}>sessions</span></div>
            <div style={{ fontSize: '1rem', color: 'var(--text-secondary)' }}>{(todayFocusMinutes / 60).toFixed(1)} hours today</div>
          </div>
        </GlassPanel>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', flex: 1, minHeight: '350px' }}>
            ...
            {weather ? (
              <div style={{ marginTop: '16px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginTop: '20px', paddingTop: '16px', borderTop: '1px solid var(--glass-border)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Sunrise</span>
                    <span style={{ fontWeight: '500' }}>{weather.sunrise}</span>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Sunset</span>
                    <span style={{ fontWeight: '500' }}>{weather.sunset}</span>
                  </div>
                </div>
              </div>
            ) : weatherError ? (
